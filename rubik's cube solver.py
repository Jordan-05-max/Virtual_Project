...
print("")
print("")
print(f"{GRAY} Please refer preview window for which side you have scanned and which color should be in centre on "
      f"each side.")
...
sticker = {'main': [
    ...
    [200, 220], [300, 120], [400, 120],
    [200, 320], [300, 120], [400, 120]
], 'current': [
    [20, 20], [54, 20], [88, 20],
    [20, 54], [54, 54], [88, 54],
    [20, 88], [54, 88], [88, 88]
...
textPoints = {
    ...
    'front': [['F', 380, 354], ['G', (0, 255, 0), 260, 360]],
...
def draw_stickers(frame, stickers, name):
    for x, y in stickers[name]:
        # color_names = state[name][stickers[name].index([x, y])]
...
if __name__ == '__main__':
    ...
    while True:
        ...
        draw_stickers(img, sticker, 'main')
        draw_stickers(img, sticker, 'current')
        draw_preview_stickers(preview, sticker)
        fill_stickers(preview, sticker, state)
        texton_preview_stickers(preview, sticker)

        for i in range(9):
            hsv.append(frame[sticker['main'][i][1] + 10][sticker['main'][i][0] + 10])

        a = 0
        for x, y in sticker['current']:
            color_name: str = color_detect(hsv[a][0], hsv[a][1], hsv[a][2])
            if 0 <= x < img.shape[1] and 0 <= y < img.shape[0]:
                cv2.rectangle(img, (x, y), (x + 30, y + 30), color[color_name], -1)
            else:
                print(f"Rectangle coordinates ({x},  {y}) are outside the image dimensions.")
            a += 1
        ...
        if k == ord('q'):
            ...
            if len(set(check_state)) == 6:
                try:
                    ...
                    print(
                        "error in side detection, you may do not follow the sequence or some color not detected well. \n "
                        "Please try again")
            else:
